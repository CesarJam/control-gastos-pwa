    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div v-else class="grid grid-cols-1 gap-4">
        <div 
          ...
          :key="item.id" 
          class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-100 dark:border-gray-700 p-5 transition-colors duration-200 flex flex-col"
        >
          <div class="flex justify-between items-center mb-3">
            ...
            <div class="flex gap-1">
              <button @click="editarMovimiento(item)" class="text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors p-1.5 bg-gray-50 dark:bg-gray-700 rounded-md" title="Editar">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
              </button>
              <button @click="eliminarMovimiento(item.id)" class="text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors p-1.5 bg-gray-50 dark:bg-gray-700 rounded-md" title="Eliminar">
          ...
          <div class="flex justify-between items-start mb-4 gap-4">
            <h4 class="text-lg font-bold text-gray-800 dark:text-white break-words leading-tight">{{ item.concepto }}</h4>
            <span class="text-xl font-black whitespace-nowrap" :class="item.tipo === 'ingreso' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
              {{ item.tipo === 'ingreso' ? '+' : '-' }}${{ item.monto.toFixed(2) }}
            </span>
          </div>

          <div class="flex justify-between items-center border-t border-gray-100 dark:border-gray-700 pt-4 mt-auto">
            <div class="flex flex-wrap gap-2">
              <span :class="item.tipo === 'ingreso' ? 'text-green-700 bg-green-100 dark:bg-green-900 dark:text-green-300' : 'text-red-700 bg-red-100 dark:bg-red-900 dark:text-red-300'" class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider">
                {{ item.tipo }}
              </span>
              <span v-if="item.tipo === 'gasto'" :class="item.estatus ? 'text-blue-700 bg-blue-100 dark:bg-blue-900 dark:text-blue-300' : 'text-yellow-700 bg-yellow-100 dark:bg-yellow-900 dark:text-yellow-300'" class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider">
            ...
            <button 
              ...
              class="flex items-center gap-1.5 text-sm font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 px-3 py-1.5 rounded-md transition-colors" 

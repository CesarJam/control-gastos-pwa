<template>
  <div
    class="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 pb-24 md:p-8 transition-colors duration-200 select-none"
  >
    <div class="max-w-5xl mx-auto">
      <div class="flex justify-between items-center mb-4 relative">
        ...
        <button
          @click="mostrarMenuOpciones = !mostrarMenuOpciones"
          class="md:hidden p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-md transition-colors"
        >
          <svg
            class="w-6 h-6"
            fill="none"
        ...
        <div
          v-if="mostrarMenuOpciones"
          class="absolute top-14 right-0 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-xl p-2 flex flex-col gap-1 z-50 md:hidden min-w-[180px]"
        >
          <button
            @click="
              toggleDark();
            ...
            "
            class="flex items-center gap-3 p-3 w-full text-left text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors"
          >
            <svg
              v-if="!isDark"
          ...
          <button
            @click="
              ...
              mostrarMenuOpciones = false;
            "
            class="flex items-center gap-3 p-3 w-full text-left text-sm font-medium text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-lg transition-colors"
          >
            <svg
              class="w-5 h-5"
              fill="none"
      ...
      <!--Filtro de búsqueda-->
      <div
        class="bg-white dark:bg-gray-800 p-1 md:p-4 rounded-lg shadow-sm mb-4 flex flex-col gap-3 transition-colors duration-200"
      >
        <div class="flex items-center gap-2 w-full">
          <div class="relative flex-1">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Buscar concepto (ej. Gasolina...)"
              class="w-full pl-10 pr-4 py-2 text-sm md:text-base border dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white rounded-lg focus:ring-blue-500 focus:border-blue-500 transition-colors"
            />
          </div>

          <button
            @click="mostrarFiltros = !mostrarFiltros"
            class="md:hidden shrink-0 p-2.5 bg-gray-100 dark:bg-gray-700/50 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg text-gray-700 dark:text-gray-300 transition-colors"
            title="Mostrar filtros de fecha"
          >
            <svg
              class="w-5 h-5"
              fill="none"
        ...
        <div class="flex flex-col md:flex-row md:items-end gap-2 md:gap-4">
          <div
            class="w-full md:flex-1 gap-2 md:gap-4 md:items-end"
            :class="
              mostrarFiltros
                ? 'grid grid-cols-4'
          ...
          >
            <div class="md:flex-none">
              <label
                class="hidden md:block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                >Año</label
              >
              <select
                v-model="filtro.anio"
                class="w-full border dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-md px-2 md:px-3 py-2 text-sm md:text-base focus:ring-blue-500"
              >
                <option
          ...
          <div class="md:ml-auto shrink-0">
            <button
              @click="abrirModal()"
              class="fixed bottom-6 right-4 md:static z-40 bg-blue-600 hover:bg-blue-700 text-white font-bold w-14 h-14 md:w-auto md:h-auto md:py-2 md:px-4 rounded-full md:rounded shadow-2xl md:shadow-none transition-all flex items-center justify-center gap-2"
            >
              <svg
                class="w-7 h-7 md:hidden"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                xmlns="http://www.w3.org/2000/svg"
      ...
      <!--Balance-->
      <div class="grid grid-cols-3 gap-2 md:gap-6 mb-2">
        <div
          @click="filtroTipo = 'ingreso'"
          class="cursor-pointer p-2 md:p-3 rounded-xl border-b-4 md:border-b-0 md:border-l-4 border-green-500 transition-all duration-300 flex flex-col items-center md:items-start justify-center"
          :class="
            filtroTipo === 'ingreso'
        ...
        >
          <div
            class="flex items-center gap-2 mb-1 md:mb-0 text-gray-500 dark:text-gray-400"
          >
            <svg
            ...
            <h3 class="hidden md:block text-sm font-semibold uppercase">
              Ingresos
            </h3>
          </div>
          <p
            class="text-sm sm:text-base md:text-xl font-bold text-green-600 dark:text-green-400 truncate w-full text-center md:text-left"
          >
        ...
        <div
          @click="filtroTipo = 'gasto'"
          class="cursor-pointer p-2 md:p-3 rounded-xl border-b-4 md:border-b-0 md:border-l-4 border-red-500 transition-all duration-300 flex flex-col items-center md:items-start justify-center"
          :class="
            filtroTipo === 'gasto'
              ? 'bg-white dark:bg-gray-800 shadow-lg scale-105 -translate-y-1 opacity-100 z-10'
              : 'bg-gray-50 dark:bg-gray-800/60 shadow-sm scale-95 opacity-50 hover:opacity-70'
        ...
        >
          ...
          <p
            class="text-sm sm:text-base md:text-xl font-bold text-red-600 dark:text-red-400 truncate w-full text-center md:text-left"
          >
        ...
        <div
          ...
          class="cursor-pointer p-2 md:p-3 rounded-xl border-b-4 md:border-b-0 md:border-l-4 transition-all duration-300 flex flex-col items-center md:items-start justify-center"
          :class="[
        ...
        >
          ...
          >
            ...
            <h3
              class="hidden md:block text-sm font-semibold uppercase truncate"
            >
              Balance
            </h3>
          </div>
          <p
            class="text-sm sm:text-base md:text-xl font-bold truncate w-full text-center md:text-left"
            :class="
              balance >= 0
      ...
      <div
        v-else-if="listaMovimientos.length === 0"
        class="p-2 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-lg shadow-sm"
      >
      ...
      <div v-else class="grid grid-cols-1 gap-3">
        ...
        >
          <div
            ...
            :class="
              item.tipo === 'ingreso'
                ? categoriasMap.ingreso.color
                : categoriasMap[item.categoria]?.color ||
                  categoriasMap.otro.color
            "
          >
            <img
              :src="
                item.tipo === 'ingreso'
                  ? categoriasMap.ingreso.imgUrl
                  : categoriasMap[item.categoria]?.imgUrl ||
                    categoriasMap.otro.imgUrl
              "
              :alt="item.categoria"
          ...
          <div class="flex-grow min-w-0">
            <div class="flex items-center gap-2">
              <p
                class="text-base font-bold text-gray-800 dark:text-white leading-tight break-words"
              >
                {{ item.concepto }}
              </p>
            </div>
            <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">
              {{
                item.tipo === "ingreso"
                  ? "Abono"
                  : categoriasMap[item.categoria]?.texto || "Gasto"
              }}
              • {{ item.fecha }}
            </p>
          </div>
          ...
          <div class="text-right flex flex-col items-end gap-1">
            ...
            <div class="flex gap-2">
              <button
                @click="abrirComprobante(item.comprobante_url)"
              ...
              <button
                ...
                class="text-gray-400 hover:text-rose-600 p-1"
              >
                <svg
                  class="w-4 h-4"
                  fill="none"
    ...
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl transform transition-all text-center"
      >
        <div
          class="w-16 h-16 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4 text-red-500"
        >
          <svg
            class="w-8 h-8"
            fill="none"
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">
          Esta acción no se puede deshacer. El registro se borrará
...
const fetchMovimientos = async () => {
  ...
  const datosLocales = await db.gastos
    .where("user_id")
    .equals(user.id)
    .toArray();
  movimientos.value = datosLocales.sort(
  ...
  if (navigator.onLine) {
    try {
      ...
      const { data: datosNube, error } = await supabase
        .from("gastos")
        .select("*")
        .eq("user_id", user.id);
      ...
      if (!error && datosNube) {
        ...
        const datosActualizados = await db.gastos
          .where("user_id")
          .equals(user.id)
          .toArray();
        movimientos.value = datosActualizados.sort(
          (a, b) => new Date(b.fecha) - new Date(a.fecha),
        );
      }
    } catch (error) {
...
const movimientosFiltrados = computed(() => {
  return movimientos.value.filter((item) => {
    ...
    const coincideFecha =
      fechaItem.getFullYear() === filtro.value.anio &&
    ...
    const coincideBusqueda = item.concepto
      .toLowerCase()
...
const categoriasMap = {
  entretenimiento: {
    ...
    imgUrl:
      "https://img.icons8.com/?size=100&id=vzJRN9S0Db0Q&format=png&color=000000",
  },
  automovil: {
  ...
  salud: {
    ...
    texto: "Salud",
    imgUrl:
      "https://img.icons8.com/?size=100&id=trsCZxEJArXb&format=png&color=000000",
  },
  gimnasio: {
    color: "bg-zinc-200 dark:bg-zinc-800/50",
    texto: "Gimnasio",
    imgUrl:
      "https://img.icons8.com/?size=100&id=eqJxioVaWa0N&format=png&color=000000",
  },
  comida: {
    color: "bg-green-100 dark:bg-green-900/30",
    texto: "Comida",
    imgUrl:
      "https://img.icons8.com/?size=100&id=as4wAkWJZrIK&format=png&color=000000",
  },
  servicios: {
    color: "bg-yellow-100 dark:bg-yellow-900/30",
    texto: "Servicios",
    imgUrl:
      "https://img.icons8.com/?size=100&id=zqNjrHRmmsQI&format=png&color=000000",
  },
  ingreso: {
    color: "bg-emerald-100 dark:bg-emerald-900/30",
    texto: "Ingreso",
    imgUrl:
      "https://img.icons8.com/?size=100&id=yUTNKgUuTlsA&format=png&color=000000",
  },
  otro: {
    color: "bg-slate-100 dark:bg-slate-900/30",
    texto: "Otro",
    imgUrl:
      "https://img.icons8.com/?size=100&id=hANsGH9seK9Q&format=png&color=000000", // Icono genérico de caja
  },
};
</script>
